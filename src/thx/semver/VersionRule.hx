    ...
    var ors = s.split("||").map((comp) -> {
      ...
      return if (p.length == 1) {
        ...
        if (p.length == 1) {
          ...
          else if (!VERSION.match(comp)) throw 'invalid single pattern "$comp"';
          else {
            // one term pattern
          ...
          if (!VERSION.match(p[0])) throw 'left hand parameter is not a valid version rule "${p[0]}"';
          var lp  = VERSION.matched(1);
          var lva = versionArray(VERSION);
          var lvf = lva.concat([0, 0, 0]).slice(0, 3);
          var lv  = Version.arrayToVersion(lvf).withPre(VERSION.matched(5), VERSION.matched(6));

          if (lp != ">" && lp != ">=") throw 'invalid left parameter version prefix "${p[0]}", should be either > or >=';
          if (!VERSION.match(p[1])) throw 'left hand parameter is not a valid version rule "${p[0]}"';
          var rp  = VERSION.matched(1);
          var rva = versionArray(VERSION);
          var rvf = rva.concat([0, 0, 0]).slice(0, 3);
          var rv  = Version.arrayToVersion(rvf).withPre(VERSION.matched(5), VERSION.matched(6));
          if (rp != "<" && rp != "<=") throw 'invalid right parameter version prefix "${p[1]}", should be either < or <=';

          AndRule(
            lp == ">" ? GreaterThanVersion(lv) : GreaterThanOrEqualVersion(lv),
            rp == "<" ? LessThanVersion(rv) : LessThanOrEqualVersion(rv)
          );
        } else throw 'invalid multi pattern $comp';
      } else if(p.length == 2) {
        if (!VERSION.match(p[0])) throw 'left range parameter is not a valid version rule "${p[0]}"';
        if (VERSION.matched(1) != null && VERSION.matched(1) != "") throw 'left range parameter should not be prefixed "${p[0]}"';
        var lv = Version.arrayToVersion(versionArray(VERSION).concat([0, 0, 0]).slice(0, 3)).withPre(VERSION.matched(5), VERSION.matched(6));
        if (!VERSION.match(p[1])) throw 'right range parameter is not a valid version rule "${p[1]}"';
        if (VERSION.matched(1) != null && VERSION.matched(1) != "") throw 'right range parameter should not be prefixed "${p[1]}"';
        var rva = versionArray(VERSION);
        var rv = Version.arrayToVersion(rva.concat([0, 0, 0]).slice(0, 3)).withPre(VERSION.matched(5), VERSION.matched(6));
      ...
      } else throw 'invalid pattern "$comp"';
    });
